...
(function (base, $) {
  ...
  var MIMETYPES = {
    'mp4': ([
      ...
      , 'mp4v.20.240'
      , 'mp4a.40.2'
    ]).map(codec => `video/mp4; codecs="${codec}, mp4a.40.2"`),
    'ogg': [
  ...
  function __runCallback() {
    if (callbackArray.length) {
      ...
      if (callback.fn instanceof Function) (callback.fn).call(callback.scope);
      this.__runCallback();
    }
  }
  ...
  function detectFullscreenSupport() {
    ...
    if (!video) return obj;
    if (video.requestFullscreen) obj.mode = { Request: 'requestFullscreen', Exit: 'exitFullscreen' };
    else if (video.webkitRequestFullscreen) obj.mode = { Request: 'webkitRequestFullscreen', Exit: 'webkitExitFullscreen' };
    else if (video.mozRequestFullScreen) obj.mode = { Request: 'mozRequestFullScreen', Exit: 'mozCancelFullScreen' };
    else if (video.msRequestFullscreen) obj.mode = { Request: 'msRequestFullscreen', Exit: 'msExitFullscreen' };
    ...
    try {
      if (canPlay) {
        ...
        for (var mimeType in MIMETYPES) {
          ...
          var _bool = (arr.some(val => !!(v.canPlayType(val).replace(/^no$/, ''))));
          var canPlayArr = arr.map(val => ({
  ...
  function testHTML5Autoplay() {
    ...
    if (!('autoplay' in video)) return failedResponse();
    ...
    try {
      if (context.HTML5VIDEO.mp4) video.src = 'data:video/mp4;base64,AAAAHGZ0eXBtcDQyAAAAAG1wNDJpc29tYXZjMQAAAz5tb292AAAAbG12aGQAAAAAzaNa'
  ...
  function hasFlash() {
    ...
    if (_detected) {
      ...
      setTimeout(() => {
        ...
        if (_body.fake && _body.parentNode) _body.parentNode.removeChild(_body);
      }, 10);
    }
  }
