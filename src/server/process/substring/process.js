...
const substringPath = `${__dirname}/js/data/substr/${substringJson}`;
...
if (cluster.isMaster) {
  ...
  process.on('message', (data) => {
    console.log(`${dt(Date.now() - then)} \tWorker ${pid} receives message`, data.nextWorker.id);
...
function temp() {
  ...
  for (let i = 0; i < queries.length; i += 1) {
    const v = queries[i];
    file.write(`${JSON.stringify([i, s.slice(v[0], v[1] + 1), Array((v[1] - v[0]) + 1)])}\n`);
  }
