...
(function(base, $) {
  ...
  function resizeElement(jqObj, ratio, dim) {
    ...
    wait.throttle(() => {
      ...
      // this.console().warn('_Utils.resizeElement', jqObj, ele[revDim](),
      // ratio, calcRatio); ele[revDim](calcRatio);
  ...
  function extend(...objs) {
    ...
    const extended = {};
    let deep = false;
    let _arguments = Array.prototype.slice.call(objs);

    // check if deep merge
    if (Object.prototype.toString.call(objs[0]) === '[object Boolean]') {
      deep = objs[0];
      _arguments = Array.prototype.slice.call(objs, 1);
    }
  ...
  function isString(stringObj) {
    return (typeof stringObj === 'string');
  }
