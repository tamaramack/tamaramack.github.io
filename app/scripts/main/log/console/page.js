...
define(['_Console'], (_Console) => {
  function PageConsole(owner) {
    ...
    if (owner && owner.data) owner.data.callback.addListener(owner.data.callback.mode, (e) => {
  ...
  function scope(owner, ...args) {
    ...
    if (typeof owner === 'string') {
      ...
      while (len--) if ((this._scopes[len]).id === owner
    ...
    if (!(owner instanceof _Console)) owner = new _Console(owner, args[0], this);
    // console.debug('NEW CONSOLE SCOPE', owner);
    else if (arguments.length > 1) owner.reload(...args);
  ...
  function reloadAll(mode) {
    var len = this._scopes.length;
    this.reload(mode);
    while (len--) (this._scopes[len]).reload(mode);
  }
});
