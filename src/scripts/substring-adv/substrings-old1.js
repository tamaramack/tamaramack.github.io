...
function countSubstrings(s, queries) {
  ...
  while (pos--) {
    ...
    if (startObj.hasOwnProperty(start)) {
      if ((startObj[start]).hasOwnProperty(end)) {
...
function countstrings(s, queries) {
  ...
  if (queriesTotal > 3000) {
    splitArray(queries, 200, matrix);
  } else if (queriesTotal > 100) {
    splitArray(queries, 10, matrix);
  } else {
    matrix.push(queries);
  }
...
function mapSubstring(s, stringTotal) {
  ...
  const stringArr = [];
  if (stringTotal > 3000) {
    splitArray(s, 200, stringArr);
  } else if (stringTotal > 100) {
    splitArray(s, 10, stringArr);
  } else {
    stepString(s, true);
  }
